...
export default function RecycleBin() {
  ...
  useEffect(() => {
    const q = query(
      ...
      where("deleted", "==", true) // Only deleted notes
    );

    const unsubscribe = onSnapshot(q, (snapshot) => {
      const deletedNotes = snapshot.docs
        .map(doc => ({ id: doc.id, ...doc.data() }))
        .sort((a, b) => b.updatedAt?.toMillis() - a.updatedAt?.toMillis()); // Sort locally
  ...
  const deletePermanently = async (id) => {
    try {
      await updateDoc(doc(db, "notes", id), { deleted: false, permanentlyDeleted: true });
      // Or use deleteDoc(doc(db, "notes", id)) to remove entirely
    } catch (err) {
      console.log("Error deleting note permanently:", err.message);
    }
  };
